/* ===============================================================================
 ...
 15. 【学科系数派生】：[学科系数] 为投影层即时派生的绩效调整因子，判定依据为开单环节
     （[开单科室代码] = 36 或 [开单科室] = '小儿科'）命中输出 1.1、否则 1.0。
     该列由事实层基础列直接决定，必须与 [开单科室代码] / [开单科室] 一同进入 GROUP BY，
     使投影表达式与分组维度严格同构；严禁在 CTE 内预物化后跨层传递，避免破坏
     现有 CTE 链路骨架。分支结果统一 CAST 至 DECIMAL(18,8)，与全链路精度对齐。
  ...
  修改日志：
  2026-09-15 14:20:00 | 字段扩展 | 新增派生列 [学科系数]：以事实层 [开单科室代码] = 36 或 [开单科室] = '小儿科' 为
                                  判定依据，命中输出 1.1、否则 1.0，CASE 分支统一 CAST 至
                                  DECIMAL(18,8)；该表达式因引用事实层基础列，同步收敛进 GROUP BY
                                  以保持投影与分组口径严格一致。CTE 结构与 LEFT JOIN 桥接主干零改动。
...
SELECT
   ...
   ,CAST(f.[开单时间] AS DATE)                                 AS [开单日期]
    ...
    -- ── 绩效调整因子（学科系数：由开单科室派生，命中儿科目输出 1.1、否则 1.0） ──
   ,CAST(
        CASE
            WHEN f.[开单科室代码] = 36 OR f.[开单科室] = N'小儿科'
            THEN 1.1
            ELSE 1.0
        END
        AS DECIMAL(18,8)
