/* ===============================================================================
  ...
  业务说明: 门诊诊察类项目（1043）按日汇总执行数据。
            依据执行科室代码 (36) 进行源头硬隔离（仅保留 36），供下游按执行核算单元汇总消费。
            投影粒度已剥离开单环节（科室/人员/日期 7 维）与对应映射链路，仅保留执行环节维度。
  学科系数: 恒定 1.1（源头已隔离，投影层常量化）

  ── 硬约束（禁止回退） ──
  执行科室判定仅允许使用事实层 [执行科室代码]（BIGINT 代理主键 id），严禁引入 [执行科室] 中文名称比较
  （如 = N'小儿科'）。名称属前端/HIS 可维护文案，一旦改名即静默失效且不报错。

  修改日志：
  2026-09-15 19:30:00 | 隔离维度切换 | 业务隔离条件由 f.[开单科室代码] 切换为 f.[执行科室代码]（小儿科代码 36）。
  ...
  2026-09-15 17:00:00 | 编码唯一性审计 | 全文件复核确认 SQL 区零 N'小儿科' 文本判定（WHERE 仅
                                   f.[开单科室代码] = 36），头部追加硬约束条款锁定该口径防回退。
  2026-09-15 16:40:00 | 语法修复与精简 | 头部 17 条规范说明压缩为极简说明块；开单科室判定废除中文
                                  名称匹配，收敛为仅编码 [开单科室代码] = 36；复核确认单语句严格闭合。
  2026-09-15 16:20:00 | 脚本拆分 | 由 门诊诊察类项目执行积分.sql 派生【小儿科分支】独立脚本，
                                 [学科系数] 简化为 CAST(1.1 AS DECIMAL(18,8)) 常量。
=============================================================================== */
...
-- 源头物理隔离：仅以执行科室代理主键 id 保留小儿科 36（与姊妹脚本严格互补）
WHERE f.[执行科室代码] = 36
GROUP BY
    f.[来源]
   ,f.[项目大类]
